...
class FoodLoggingControllerApi {
  ...
  Future<Response> getDashboardWithHttpInfo({
    ...
    if (date != null) {
      queryParams.addAll(
          _queryParams('', 'date', DateFormat('yyyy-MM-dd').format(date)));
    }
  ...
  Future<List<RecipeResponseDto>?> getUserRecipes({
    ...
    if (response.body.isNotEmpty &&
        response.statusCode != HttpStatus.noContent) {
      final decoded = await _decodeBodyBytes(response);
      final jsonList = json.decode(decoded) as List;
      return jsonList
          .map((e) => RecipeResponseDto.fromJson(e as Map<String, dynamic>))
          .whereType<RecipeResponseDto>()
          .toList();
    }
    return null;
  }
  ...
  Future<Response> searchRecipesWithHttpInfo(
    String query,
  ) async {
    // ignore: prefer_const_declarations
  ...
  ///
  /// * [String] query (required):
  Future<List<RecipeResponseDto>?> searchRecipes(
    String query,
  ) async {
    final response = await searchRecipesWithHttpInfo(
      query,
    );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
