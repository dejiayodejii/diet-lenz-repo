//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:diet_lenz_api/api.dart';
import 'package:test/test.dart';

// tests for MealLogResponseDto
void main() {
  // final instance = MealLogResponseDto();

  group('test MealLogResponseDto', () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String recipeId
    test('to test the property `recipeId`', () async {
      // TODO
    });

    // String foodName
    test('to test the property `foodName`', () async {
      // TODO
    });

    // String mealType
    test('to test the property `mealType`', () async {
      // TODO
    });

    // DateTime loggedDate
    test('to test the property `loggedDate`', () async {
      // TODO
    });

    // String loggedTime
    test('to test the property `loggedTime`', () async {
      // TODO
    });

    // double servingMultiplier
    test('to test the property `servingMultiplier`', () async {
      // TODO
    });

    // ConsumedMacrosDto consumedMacros
    test('to test the property `consumedMacros`', () async {
      // TODO
    });

    // String notes
    test('to test the property `notes`', () async {
      // TODO
    });

    // bool isFavorite
    test('to test the property `isFavorite`', () async {
      // TODO
    });


  });

}
