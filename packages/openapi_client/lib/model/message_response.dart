...
class MessageResponse {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "MessageResponse[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "MessageResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, List<MessageResponse>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = MessageResponse.listFromJson(
          entry.value,
          growable: growable,
        );
      }
