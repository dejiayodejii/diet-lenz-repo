...
class Goals {
  ...
  @override
  String toString() =>
      'Goals[currentWeight=$currentWeight, targetWeight=$targetWeight, kgToGo=$kgToGo, weightUnit=$weightUnit, projectedGoalDate=$projectedGoalDate, headline=$headline]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Goals[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Goals[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Goals> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <Goals>[];
  ...
  static Map<String, List<Goals>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Goals.listFromJson(
          entry.value,
          growable: growable,
        );
      }
